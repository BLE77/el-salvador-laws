#!/usr/bin/env python3
"""
El Salvador Law Agent v2 — Three-layer search architecture.

Search layers (tried in order):
    1. Wiki Layer    — Pre-compiled markdown wiki pages (fastest, best quality)
    2. QMD Hybrid    — BM25 + vector + LLM reranking via QMD CLI
...
Usage:
    python scripts/serve_v2.py
    python scripts/serve_v2.py --port 8080
    python scripts/serve_v2.py --db B:/el-salvador-laws/db/laws.db
    python scripts/serve_v2.py --wiki B:/el-salvador-laws/wiki
"""
...
# ─── Layer availability flags (detected at startup) ───
_wiki_available = False
_wiki_index = {}       # slug -> {"path": Path, "title": str, "tags": [...], "body": str}
        ...
        if len(parts) >= 3:
            ...
            for line in parts[1].strip().splitlines():
                ...
                if ":" in line:
                    ...
                    key = key.strip()
                    val = val.strip()
                    # Handle YAML lists on one line: [a, b, c]
...
def init_wiki():
    ...
    Each .md file is parsed for frontmatter (title, tags, decree, aliases)
    and the full body text is kept for content matching."""
    ...
    for md_file in WIKI_DIR.rglob("*.md"):
        ...
        # Collect all searchable text: title, tags, aliases, filename
...
def init_qmd():
    """Check if the QMD CLI tool is available."""
    ...
    try:
        ...
        if _qmd_available:
            ...
            print(f"  [qmd]  Not available (exit {result.returncode})")
...
def search_wiki(question, limit=3):
    """Search pre-compiled wiki pages using keyword matching.

    Strategy:
        1. Check for decreto number match (exact)
        2. Score each wiki page by keyword overlap with the question
        3. Return top matches with their full content

    Returns list of dicts: {"title", "decreto", "content", "score", "source": "wiki"}
    """
    ...
    # Extract search tokens from the question
    ...
    for slug, entry in _wiki_index.items():
        ...
        # Exact decreto number match = highest priority
        if target_decreto and entry["decreto"] == target_decreto:
            score += 100.0

        # Build searchable text from page metadata + body
        searchable = " ".join([
            entry["title"].lower(),
            slug,
            ...
            entry["body"][:2000].lower(),  # Only search first 2K chars of body for speed
        ])

        # Token matching: count how many query tokens appear in the page
        matches = 0
        ...
        if expanded_tokens:
            # Proportion of query tokens that matched
            token_score = matches / len(expanded_tokens)
            score += token_score * 50.0

        # Bonus: title/slug match is worth more than body match
        ...
        if score > 15.0:  # Minimum threshold to be considered a match
            # Prepare content — full body, will be truncated later by context builder
            content = entry["body"]
            results.append({
                "title": entry["title"],
...
def search_qmd(question, limit=5):
    """Query the QMD hybrid search engine via CLI.

    Shells out to: qmd query "<question>" --collection <name> --json --limit <N>

    QMD combines BM25 + vector similarity + optional LLM reranking for
    high-quality semantic search over the markdown document corpus.

    Returns list of dicts: {"title", "content", "score", "source": "qmd", ...}
    Falls back gracefully if QMD is not installed or errors out.
    """
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"  [qmd]  Query error (exit {result.returncode}): {result.stderr[:200]}")
            return []

        # Parse QMD JSON output
        # Expected format: {"results": [{"title": ..., "content": ..., "score": ..., "metadata": {...}}]}
        data = json.loads(result.stdout)

        qmd_results = []
        items = data if isinstance(data, list) else data.get("results", [])
        for item in items:
            # Normalize QMD output to our internal format
            ...
            # Extract decreto number and year from file path or title
            # File format: qmd://el-salvador-laws/asamblea-year-archive-2003-decreto-153.md
            decreto = ""
            year = ""
...
# ─── Query expansion (shared by FTS layer) ───
...
def expand_query(question):
    ...
    # 3. Direct query (cleaned for FTS5)
...
def search_fts_expanded(question, limit=12):
...
def smart_search(question, limit=12):
    """Three-layer search orchestrator.

    Order:
        1. Wiki — pre-compiled pages, best quality, no LLM needed for simple lookups
        2. QMD  — semantic hybrid search (BM25 + vector + reranking)
        3. FTS5 — keyword fallback, always available

    Results are tagged with their source layer, deduplicated by decreto number,
    and merged into a single ranked list.

    Returns:
        {
            "wiki_results": [...],      # Raw wiki matches
            "qmd_results": [...],       # Raw QMD matches
            "fts_results": [...],       # Raw FTS matches
            "merged": [...],            # Deduplicated, ranked, merged results
            "layers_used": ["wiki", "qmd", "fts"],
            "wiki_hit": bool,           # True if wiki had a strong match
        }
    """
    wiki_results = search_wiki(question, limit=3)
    qmd_results = search_qmd(question, limit=5)
    fts_results = search_fts_expanded(question, limit=limit)

    # Determine if wiki had a strong hit (score > 50 means good keyword overlap)
    wiki_hit = len(wiki_results) > 0 and wiki_results[0]["score"] > 50.0

    # Merge and deduplicate by decreto number
    seen_decretos = set()
    merged = []

    # Priority 1: Wiki results (pre-compiled, highest quality)
    ...
    # Priority 3: FTS results (keyword fallback)
    for fr in fts_results:
        ...
        merged.append({
            ...
            "decreto": decreto,
            "content": text,
            "score": -fr.get("relevance", 0),  # FTS rank is negative (lower = better)
            "year": fr.get("year", ""),
    ...
    # Sort merged: active > unknown > repealed (within each layer priority group)
...
def get_stats():
    ...
    stats = {
        ...
        # v2: include layer availability
        "search_layers": {
            "wiki": _wiki_available,
            "wiki_pages": len(_wiki_index),
            "qmd": _qmd_available,
            "fts": True,  # Always available
        },
    }
    conn.close()
    return stats
...
7. If a law is marked as REPEALED, warn the user clearly and try to cite the replacement law instead. Prefer citing active laws over repealed ones.
...
def agent_chat(question):
    ...
    Flow:
        1. Check for direct decreto lookup
        2. Run three-layer smart_search
        3. If wiki has a strong hit, use wiki content directly (may skip LLM for factual queries)
        4. Build context from merged results (max 3 sources, 4K chars for Kimi)
        5. Call LLM for synthesis
        6. Return answer with sources and layer metadata
    """
    ...
    search_result = smart_search(question, limit=12)
    ...
    if not merged and not decree_data:
        ...
        for word in words:
            if len(word) > 3:
                fts_fallback = search_fts(word, limit=5)
                if fts_fallback:
    ...
    # Add merged results from the three layers
    for item in merged:
    ...
    # Step 5: AGGRESSIVE context limiting for Kimi K2.5 (max 4K chars, max 3 sources)
    MAX_SOURCES = 3
    MAX_PER_SOURCE = 1200
    MAX_TOTAL = 4000
    ...
    # For strong wiki hits, tell the LLM this is pre-compiled reference material
    source_note = ""
    if wiki_hit:
...
<main>
  <div id="messages">
    <div class="welcome">
      ...
      <p style="font-size:0.78rem; color:var(--text-dim)">Ask in English or Spanish. Three-layer search: Wiki + Semantic + Keyword.</p>
      <div class="examples">
  ...
  try {
    ...
    if (data.layers_used && data.layers_used.length > 0) {
      ...
      for (const layer of data.layers_used) {
        html += '<span class="layer-tag ' + layer + '">' + layer + '</span>';
      }
    ...
    if (data.top_results && data.top_results.length > 0) {
      ...
      for (const r of data.top_results) {
        ...
        const layerTag = r.layer ? ' <span class="layer-tag ' + r.layer + '" style="font-size:0.55rem;">' + r.layer + '</span>' : '';
        html += '<div style="color:var(--text-secondary);padding:0.15rem 0;">Decreto ' + r.decree_no + ' (' + (r.year || '?') + ')' + layerTag;
        if (r.materia) html += ' &mdash; ' + r.materia;
    ...
    if (data.sources && data.sources.length > 0) {
      ...
      for (const s of data.sources) {
        ...
        label += ' (' + (s.year || '?') + ')';
        const layerClass = s.layer || 'fts';
        let meta = '';
        if (s.materia) meta += s.materia;
        if (s.rama) meta += (meta ? ' / ' : '') + s.rama;
        if (s.emission_date) meta += (meta ? ' &mdash; ' : '') + 'Issued ' + s.emission_date;
...
def get_openapi_spec():
    return {
        ...
        "info": {
            ...
            "description": "Three-layer search (Wiki + QMD + FTS5) over the complete corpus of El Salvador laws, decrees, and legal texts. Covers decrees from the Asamblea Legislativa (1860-2026).",
            "version": "2.0.0",
        },
        ...
        "paths": {
            ...
            "/api/chat": {
                "post": {
                    ...
                    "description": "Three-layer RAG agent: Wiki (pre-compiled) -> QMD (semantic) -> FTS5 (keyword). Deduplicates, merges, and synthesizes a grounded answer with citations.",
                    "requestBody": {
                        "required": True,
                        "content": {
                            "application/json": {
                    ...
                    "responses": {"200": {"description": "Grounded answer with decree citations, source list, and layer metadata"}},
                }
            },
            "/api/stats": {
                "get": {
                    "operationId": "getCorpusStats",
                    "summary": "Get corpus statistics: document counts, categories, year coverage, search layer status",
                    "responses": {"200": {"description": "Corpus statistics including search layer availability"}},
                }
            },
        },
    }
...
def main():
    global DB_PATH, WIKI_DIR

    parser = argparse.ArgumentParser(description="El Salvador Law Agent v2 — Three-layer search")
    ...
    print()
    print("=" * 56)
    print("  El Salvador Law Agent v2 (Three-Layer Search)")
    print("=" * 56)
